...
$reset_user = !setting('admin_user', 'password', false) || (admin()->getSetup('user')['reset'] ?? false);
?>
<section class="flex items-center justify-center min-h-screen w-full">
    <div class="bg-zinc-950 rounded-b">
        ...
        <form action="<?= admin_url('login') ?>" method="post" class="px-8 py-4 text-center">
            ...
            <?php if ($reset_user) : ?>
                <input type="password" name="confirm_password" placeholder="<?= __('Confirm Password?') ?>" class="w-64 mt-4 block text-center bg-zinc-900 px-4 py-1 border border-zinc-800 rounded">
            <?php endif ?>
            <button type="submit" class="px-4 py-1 text-sm bg-teal-500 hover:bg-teal-600 border border-teal-500/75 mt-4 rounded"><?= __($reset_user ? 'Setup' : 'Login') ?></button>
        </form>
    </div>
</section>