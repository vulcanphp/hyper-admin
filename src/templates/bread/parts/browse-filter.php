    ...
    <div class="w-full md:w-4/12 lg:w-3/12 xl:w-2/12">
        ...
        <div class="bg-zinc-800">
            <?php if ($showFacts = request()->get('_facts') == 'true') : ?>
                <a href="<?= hide_filter_count() ?>" class="flex gap-1 text-sm text-slate-200 hover:text-teal-500 items-center border-b px-2 py-3 border-zinc-700">
            ...
            <?php else : ?>
                <a href="<?= show_filter_count() ?>" class="flex gap-1 text-sm text-slate-200 hover:text-teal-500 items-center border-b px-2 py-3 border-zinc-700">
                    <svg xmlns="http://www.w3.org/2000/svg" class="fill-current text-teal-600/75" width="18" height="18" viewBox="0 0 24 24">
            ...
            <?php foreach ($model->getFilter() as $field => $options) : ?>
                ...
                <div toggle="<?= $model->name() . '_filter_' . $field ?>" class="px-3 py-2 text-sm">
                    ...
                    <ul class="mt-2 px-1">
                        <li class="mb-1 text-[90%] hover:text-teal-500 <?= $filter_value === null ? 'text-indigo-400' : 'text-slate-300' ?>"><a href="<?= get_filter_url($field, false) ?>"><?= __('All') ?></a></li>
                        <?php foreach ($options as $val => $key) : ?>
                            <li class="mb-1 text-[90%] hover:text-teal-500 <?= $filter_value !== null && $filter_value == $val ? 'text-indigo-400' : 'text-slate-300' ?>"><a href="<?= get_filter_url($field, $val) ?>"><?= __($key) ?><?= $showFacts ? ' (' . $model->getModel()->where([$field => $val])->where($where)->count() . ')' : '' ?></a></li>
                        <?php endforeach ?>
                    </ul>
                </div>
            <?php endforeach ?>
