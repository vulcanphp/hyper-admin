...
<nav class="bg-teal-900 text-slate-100 text-sm lg:text-base px-4 md:px-6 h-[30px] flex items-center relative z-30">
    <div class="flex w-full items-center gap-2">
        <a fire class="opacity-75 text-sm hover:opacity-85 transition-all" href="<?= admin_url() ?>"><?= __text('Home') ?></a>
        <?php foreach ($links as $label => $url) : ?>
            <span class="opacity-85">›</span><a fire class="opacity-75 text-sm hover:opacity-85 transition-all" href="<?= $url ?>"><?= __text($label) ?></a>
        <?php endforeach; ?>
